...
def permutation_dashboard_view(request):
    ...
    return render(request, 'permutation/dashboard.html', context)
...
def get_p1_metric_data_view(request):
    if request.method == 'POST':
        try:
...
def run_p1_permutation_view(request):

    context = {}

    return render(request, 'p1/summary_result.html', context)


def current_p1_result_view(request):

    context = {}

    return render(request, 'p1/run_output.html', context)
